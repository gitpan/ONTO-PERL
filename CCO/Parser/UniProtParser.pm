...
sub work {
	...
	my $short_map_g = CCO::Util::CCO_ID_Term_Map->new($short_map_g_file);
	my $long_map_g  = CCO::Util::CCO_ID_Term_Map->new($long_map_g_file);    # Set of Gene IDs
	...
	while (<FH>) {    
		...
		if(my @fts = @{$entry->FTs->{list}}){#an array of references to arrays corresponding to individual FT lines 
			foreach my $ft (@fts){
				...
				my $mod_prot_obj = CCO::Core::Term->new();
