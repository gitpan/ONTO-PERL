								...
								if($value eq $neutral_component){
									...
									if($ebi_interactor->ncbiTaxId eq $taxon){
										...
										if (!defined $OBOed_EBI_interactor){
											$OBOed_EBI_interactor = &add_interactor($ebi_interactor,$ontology,$cco_b_taxon_ids_map,$cco_b_ids_map,$is_a,$derives_from,$taxon);
										}
...
=head2 get_xref_acc
  ...
  Args     - the database name and the term (CCO::Core::Term)
...
sub add_interactor(){
	my ($ebi_interactor_object,$ontology,$cco_b_taxon_ids_map,$cco_b_ids_map,$is_a,$derives_from,$taxon) = @_;
	...
	# Add derives_from taxon
	my $CCO_taxon = $ontology->get_term_by_xref('NCBI', $taxon);
	$ontology->create_rel($OBOed_EBI_interactor, $derives_from, $CCO_taxon);
	# Add xref
